...
async function processSummary({
    ...
    const filteredCommentsWords = filteredComments
        .map((comment) => comment.user && comment.body ? comment.user + ': ' + comment.body : '')
        .join('\n')
        .split(/\s/)
        .filter(Boolean)

    const summary = await openai.summarize(filteredCommentsWords.join(' '))

    commentReply.reply(summary)  
}
...
module.exports = { processIssueComment, processContribution, listComments }
